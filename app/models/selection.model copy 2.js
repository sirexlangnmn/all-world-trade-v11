...
Model.getAllBySearchParameter = (param, result) => {
    ...
    if (param.trade_categories) {
        query += ` OR users_business_characteristics.business_major_category = '${param.trade_categories}'`;
    }
    ...
    if (param.selectionCity) {
        query += ` OR users_businesses.business_city = '${param.selectionCity}'`;
    }
    if (param.selectionCity) {
        query += ` OR users_businesses.city_of_operation = '${param.selectionCity}'`;
    }
    if (param.language) {
        query += ` OR users_businesses.business_language_of_communication LIKE '%${param.language}%'`;
    }
    if (param.business_scale) {
        query += ` OR users_business_characteristics.business_scale = '${param.business_scale}'`;
    }
    if (param.sub_categories) {
        query += ` OR users_business_characteristics.business_sub_category = '${param.sub_categories}'`;
    }
    if (param.minor_sub_categories) {
        query += ` OR users_business_characteristics.business_minor_sub_category = '${param.minor_sub_categories}'`;
    }
    ...
    if (searchTerms.length > 0) {
        const searchConditions = searchTerms.map((term) => {
            ...
            return `(users_businesses.business_name LIKE '%${plainTerm}%'
                OR REPLACE(users_business_characteristics.business_industry_belong_to, '#', '') LIKE '%${plainTerm}%')`;
        });
        query += ` OR (${searchConditions.join(' OR ')})`;
    }
