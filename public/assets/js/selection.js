...
selectionsNextSlide.addEventListener('click', () => {
    // currentIndex = (currentIndex + 1) % companyDetailsJsonObj2[0].length;
    // handleSlideChange(currentIndex);
    handleSlideNavigation('next');
});

selectionsPrevSlide.addEventListener('click', () => {
    // currentIndex = (currentIndex - 1 + companyDetailsJsonObj2[0].length) % companyDetailsJsonObj2[0].length;
    // handleSlideChange(currentIndex);
    handleSlideNavigation('prev');
});

// function handleSlideChange(index) {
//     console.log('handleSlideChange index:', index);
//     const currentBusiness = companyDetailsJsonObj2[0][index];
...
//     console.log('Current business_name:', business_name);
//     console.log('Current banner:', banner);
//     // console.log('Full Business Data:', currentBusiness);

//     // Example usage
//     // updateBanner(currentBusiness.banner);
//     // updateName(currentBusiness.business_name);
//     // updateEmail(currentBusiness.business_email);
...
//     // Loop banners (reverse order as you intended)
...
//         console.log('adjustedHeight on load:', adjustedHeight);
//         // this.style.maxHeight = `${adjustedHeight}px`;
//         this.style.height = '100%';
//         this.style.width = '100%';
//         this.style.objectFit = 'cover'; // or 'contain' if you don’t want cropping
//     };
...
//     // Append all at once (performance-friendly)
...
//         img.style.maxHeight = `${getAdjustedHeight()}px`;
//     });
...
function handleSlideChange(index) {
    ...
    const business = getBusinessByIndex(index);
    if (!business) return;

    console.log('Current UUID:', business.uuid);
    console.log('Current business_name:', business.business_name);
...
function getBusinessByIndex(index) {
    console.log('companyDetailsJsonObj2[0] 1:', companyDetailsJsonObj2[0]);
    return companyDetailsJsonObj2?.[0]?.[index] || null;
}
...
function updateBusinessDetails(business) {
    ...
    if (business_language_of_communication) {
        formattingLanguageName(business_language_of_communication);
    }

    businessMajorCategoryId.innerHTML = getCategoryTitle(business_major_category, getTradeCategoriesTitleById);

    businessSubCategoryId.innerHTML = getCategoryTitle(business_sub_category, getSubCategoriesTitleById);

    businessMinorSubCategoryId.innerHTML = getCategoryTitle(
        business_minor_sub_category,
        getMinorSubCategoriesTitleById,
    );

    businessScaleId.innerHTML = business_scale ? getBusinessScaleTitle(business_scale) : 'N/A';

    if (business_industry_belong_to) {
        formattingBusinessTags(business_industry_belong_to);
    }

    updateOperationLocation(country_of_operation, states_of_operation, city_of_operation);
    updateOperatingTime(start_operating_hour, end_operating_hour, region_of_operation);
}

function getCategoryTitle(value, resolverFn) {
    return value ? resolverFn(value) : 'N/A';
}

function updateOperationLocation(country, states, city) {
    if (country) {
        getCountryNameUsingCode(country, 'selection-company-country-of-operations');
    }

    if (states) {
        getStatesNameToBeDisplayUsingCode(states, 'selection-company-state-of-operations');
    }

    if (city) {
        getCityNameToBeDisplayUsingCode(city, 'selection-company-city-of-operations');
    }
}

function updateOperatingTime(start, end, region) {
    companyRegionOfOperationsId.innerHTML = region || 'N/A';

    if (!start || !end) {
        localOperatingTimeId.innerHTML = 'N/A';
        uctOperatingTimeId.innerHTML = 'N/A';
        return;
    }

    localOperatingTimeId.innerHTML = `${start} - ${end}`;

    uctOperatingTimeId.innerHTML = `${convertTimeToInternationalStart(start, 'Philippines', 'UTC')} - 
         ${convertTimeToInternationalEnd(end, 'Philippines', 'UTC')}`;
}

function getSlideshowContainer() {
    return document.querySelector('[uk-slideshow]');
}

function getSlideshowComponent() {
    const container = getSlideshowContainer();
    return container ? UIkit.slideshow(container) : null;
}

function showSlide(index) {
    const slideshow = getSlideshowComponent();
    if (!slideshow) return;
    try {
        slideshow.show(index);
    } catch (error) {
        console.error('showSlide error:', error);
    }
}

function renderCompanyBanners() {
    companiesProfilePicture.innerHTML = '';
    ...
    const fragment = document.createDocumentFragment();
    const adjustedScreenHeight = window.innerHeight - HEADER_OFFSET;

    companies.forEach((company, index) => {
        const bannerTitle = getCompaniesProfilePicture(company['id'], company['uuid']);
        if (!bannerTitle) return;

        const img = new Image();
        img.className = 'companyBannerPreview';
        img.loading = 'lazy';
        img.src = host + '/uploads/users_upload_files/' + getCompanyImageFilename(company);
        img.onload = function () {
            applyCompanyImageStyles(this, adjustedScreenHeight);
        };

        const li = document.createElement('li');
        li.dataset.companyIndex = String(index);
        li.appendChild(img);
        fragment.appendChild(li);
    });
...
function highlightActiveCompany(index) {
    const items = topSelectionResultsId.querySelectorAll('.top-selection-result-item');
...
function positionNavigationButtons() {
    ...
    selectionsPrevSlide.style.top = `${centerY}px`;
    selectionsNextSlide.style.top = `${centerY}px`;
}

async function fetchAdditionalCompanies() {
    try {
        const response = await fetch('/api/get/get-companies-related-to-current-user', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({
                offset: companyDetailsJsonObj2[0].length,
                limit: 10,
            }),
        });
...
//                 <div class="flex-1">
//                     <a href="#" onclick="displayTopCompanyDetails(${i})" class="text-base font-semibold capitalize">
...
function displayTopCompany() {
    ...
    for (let i = 0; i < length; i++) {
        ...
        topSelectionResultsId.innerHTML += `
            <div class="top-selection-result-item flex items-center space-x-4 rounded-md -mx-2 p-2 hover:bg-gray-50 ${isActive ? 'bg-gray-50' : ''}">
                <div class="flex-1">
                    <a href="#"
...
//             const prevBtn = document.getElementById('selections-prev-slide');
//             const nextBtn = document.getElementById('selections-next-slide');

//             if (leng > 1) {
//                 prevBtn.style.display = currentCompanyIndex > 0 ? 'block' : 'none';
//                 nextBtn.style.display = currentCompanyIndex < leng - 1 ? 'block' : 'none';

//                 prevBtn.onclick = function (e) {
//                     e.preventDefault();
//                     if (currentCompanyIndex > 0) {
//                         let prevCompany = companyDetailsJsonObj2[0][currentCompanyIndex - 1].business_name;
//                         displayTopCompanyDetails(prevCompany);
//                     }
//                 };

//                 nextBtn.onclick = function (e) {
//                     e.preventDefault();
//                     if (currentCompanyIndex < leng - 1) {
//                         let nextCompany = companyDetailsJsonObj2[0][currentCompanyIndex + 1].business_name;
//                         displayTopCompanyDetails(nextCompany);
//                     }
//                 };
//             } else {
...
function handleSelectionPageResize() {
    ...
    const adjustedScreenHeight = screenHeight - HEADER_OFFSET;
    const adjustedScreenHeight2 = (screenHeight - HEADER_OFFSET) / 2;
...
function noRecordFoundImageSrc() {
    ...
    companiesProfilePicture.innerHTML = `<li><img src="${noRecordFoundImageSrc}" id="companyBannerPreview" alt="" uk-cover></li>`;
...
if (slideshowContainer) {
    UIkit.util.on(slideshowContainer, 'itemshown', function (e) {
        const slideshow = e.detail && e.detail[0];
        if (!slideshow || typeof slideshow.index !== 'number') return;

        const index = slideshow.index;
...
function downloadOrContact(filename) {
    ...
    let selectionQuestion = UIkit.modal('#selectionQuestion');
...
async function handleContactTrader(trader_uuid, communicator_link) {
    try {
        ...
        if (traderIsActive == 1) {
            ...
            transferTheCallToAWT(traderIsActive);
...
function transferTheCallToAWT(awt_communicator_link) {
    const domainLink = 'https://meet.allworldtrade.com/join/'; //new
    window.open(domainLink + awt_communicator_link, '_blank');
    console.log('domain link', domainLink + awt_communicator_link);
}
